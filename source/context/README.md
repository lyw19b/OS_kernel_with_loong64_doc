...
- 提供了loongsonlab命令快速初始化文档项目模板,make命令进行编译构建
...
> 具体的配置参数在loongsonlab命令生成的项目模板source/conf.py文件中，每个配置参数均已中文注释详细说明。

用户通过简单的命令即可实现项目文档模板初始化(loongsonlab命令，取代sphinx-quickstart)、文档构建(make html, make docx, make pdf以及make all)工作。
...
sphinx_loongsonlab_theme支持在windows和linux环境下构建文档，目前已在windows 11以及ubuntu 22.04上进行了测试。

sphinx_loongsonlab_theme在windows下需要安装python, perl, xelatex以及latexmk。

sphinx_loongsonlab_theme是基于sphinx创建的主题，而sphinx本身是一个python脚本/库，所以先需要安装python，建议版本为3.7以上。
...
## 如何安装Sphinx LoongsonLab Theme

sphinx_loongsonlab_theme托管在[PyPI](https://pypi.org/)中，通过python的pip安装：

```python
pip install sphinx_loongsonlab_theme
```
...
本项目的使用非常简单，只需要在任意空文件夹下执行`loongsonlab`命令，即可创建项目模板

```{image} ../_static/loongsonlab_init.jpg
:class: bg-primary
:scale: 50 %
:align: center
```
...
> 目前pdf封面左下角会显示loongsonlab版权信息，如果您需要修改，请自行fork此项目并在\sphinx_loongsonlab_theme\latex_templates\titlepage.tex 第45行修改版权信息
...
Sphinx LoongsonLab Theme已经默认开启了reStructuredText和Markdown支持
...
## Sphinx LoongsonLab Theme 目录结构

```
sphinx_loongsonlab_theme
│
│
├───sphinx_loongsonlab_theme          # 主题主目录
