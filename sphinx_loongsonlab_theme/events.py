...
def config_initiated(app: Sphinx, config: Config) -> None:
    ...
    if config['project'] == 'Python':
        config['project'] = u'龙芯实验室项目文档模板'

    if config['author'] == 'unknown':
        if config['language'] == 'zh_CN':
            config['author'] = u'龙芯中科技术股份有限公司'
        else:
            config['author'] = u'Loongson Technology Co., Ltd'

    if config['company'] == '':
        if config['language'] == 'zh_CN':
            config['company'] = u'龙芯中科技术股份有限公司'
        else:
            config['company'] = u'Loongson Technology Co., Ltd'

    if not config['copyright']:
        currentYear = datetime.now().year
        if config['language'] == 'zh_CN':
            config['copyright'] = '2022-' + str(currentYear) + ', ' + '龙芯中科技术股份有限公司'
        else:
            config['copyright'] = '2022-' + str(currentYear) + ', ' + 'Loongson Technology Co., Ltd'
...
def override_latex_config(app: Sphinx, config: Config) -> None:
    ...
    config["latex_logo"] = os.path.join(theme_path, 'latex_templates', 'logo.png')
